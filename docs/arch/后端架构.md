...
所有与系统运行相关的文件都集中存放在workspace目录下，这样能够更好地管理文件，简化部署和维护。
...
3. **模板资源复用**: 在API实现中，模板分析结果被保存为JSON文件，预览图被生成为PNG文件，可被多个生成任务复用。模板分析结果统一保存在`workspace/cache/ppt_analysis`目录下，按照模板ID组织。
...
| 数据类型 | 存储目录 | 说明 |
|------------|----------------|------|
| `模板分析结果` | `workspace/cache/ppt_analysis/{template_id}` | 模板分析结果和预览图 |
| `生成结果` | `workspace/output/{task_id}` | 生成的PPT和预览图 |
| `临时上传文件` | `workspace/uploads` | 临时上传文件存储 |
| `日志文件` | `workspace/logs` | 日志文件存储 |
| `数据库文件` | `workspace/db` | SQLite数据库文件 |

模板分析结果的文件命名使用PPTX文件名命名，便于在直接运行`run_workflow.py`和通过API上传模板时保持一致的命名逻辑。
...
│   ├── sessions/          # 会话数据
│   ├── cache/             # 缓存数据
│   │   ├── markdown/      # Markdown解析缓存
│   │   ├── ppt_analysis/  # PPT分析缓存
│   │   └── content_plan/  # 内容规划缓存
│   ├── output/            # 生成结果
│   ├── temp/              # 临时文件
│   ├── logs/              # 系统日志
