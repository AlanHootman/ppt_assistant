...
1. 内容类型与布局结构的匹配度
2. 内容数量与布局容量的适配性
3. 内容形式(文本/图片/图表)与布局元素的兼容性
4. 内容重要性与布局强调点的一致性
5. 内容逻辑与布局流程的连贯性

为每种内容类型推荐最佳的布局选择，并给出详细的匹配度评分（1-10分）与推荐理由。
请特别考虑内容中的段落数量、项目符号数量与布局的容量匹配情况。
  ...
  "recommendations": [
    {
      ...
      "matchAnalysis": {
        "structuralFit": "结构匹配度评价",
        "capacityFit": "容量匹配度评价",
        "elementFit": "元素匹配度评价",
        "emphasisFit": "强调点匹配度评价"
      },
      "reason": "详细的推荐理由，解释为什么这种布局适合该内容",
      "adaptationSuggestions": "如何调整内容以更好地适应此布局的建议"
    }
  ],
  "alternativeOptions": [
    {
      ...
      "alternativeLayouts": [
        {
          "layout": "备选布局名称或索引", 
          "score": 7,
          "tradeoffs": "使用此布局的利弊分析"
        }
      ]
    }
  ],
  "presentationFlowSuggestions": {
    "openingLayout": "建议的开场布局",
    "contentLayouts": ["内容页布局建议"],
    "closingLayout": "建议的结束布局",
    "transitionNotes": "布局之间的流畅过渡建议"
  }
}
