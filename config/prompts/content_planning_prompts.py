...
5. 必须避免重复使用同一个slide_index，每个slide_index只能分配给一个章节内容
...
【主要任务】：
你的主要任务是为每个内容章节选择最合适的slide_index，并给出选择理由。特别注意：
1. 优先选择最匹配章节内容的布局，特别是对于有items的情况，要尽量确保章节内容的items数量与template中的placeholder数量匹配
2. 每个slide_index只能使用一次，不允许重复分配
3. 如果找不到完全匹配的未使用slide_index，请提供layout名称，系统将基于layout_name创建新的幻灯片
...
[
  ...
  },
  {
    ...
    "reasoning": "所有匹配的slide_index已被使用，需要基于layout创建新幻灯片"
  },
  ...,
  {
    "slide_type": "closing",
...
8. 每个slide_index只能分配一次，如果找不到合适的未使用slide_index，设置slide_index为null，但必须提供layout名称
9. 对于有items的章节，务必确保所选布局的placeholder数量与items数量尽可能匹配
